...
class TestPlotStore:
    ...
    def test_add_scatter(make_store):
        ...
        tab_name = "scatter"
        store.add_tab(tab_name)
        store.plot_scatter(tab_name, {"x": x, "y": y})
        assert "item #0" in store.tabs[tab_name]
        store.plot_scatter(tab_name, {"x": x, "y": y})
        assert "item #1" in store.tabs[tab_name]
    ...
    def test_add_image(make_store):
        ...
        image = np.random.randint(0, 100, (10, 10))
        tab_name = "heatmap"
        store.plot_image(tab_name, {"image": [image]})
        assert "item #0" in store.tabs[tab_name]
    ...
    def test_add_rgba_image(make_store):
        ...
        tab_name = "rgba"
        store.plot_rgb_image(tab_name, {"image": rgba_img})
        store.plot_rgb_image(tab_name, {"image": [rgba_img], "intensities": [rgba_intensity]})
        assert "item #0" in store.tabs[tab_name]
    ...
    def test_add_annotations_line(make_store):
        ...
        tab_name = "plot"
        store.add_tab(tab_name)
        _, _, plot = store.plot_spectrum(tab_name, {"x": x, "y": y})
        assert "item #0" in store.tabs[tab_name]
        # band annotation
        store.add_band(plot, {"base": x, "lower": y - 3, "upper": y + 3})
        # span
        store.add_span(plot, {"location": 1, "dimension": "width"})
        store.add_span(plot, {"location": 1, "dimension": "height"})
        store.add_span(plot, {"location": [5, 3], "dimension": "height"})
        # add box
        store.add_box(plot, {"bottom": 1, "right": 3, "top": 4, "left": 2})
        with pytest.raises(ValueError):
            store.add_box(plot, {"bottom": 3, "right": 7})
            store.add_box(plot, {"top": 4, "left": 2})
        # add labels
        store.add_labels(plot, {"x": [3, 4], "y": [3, 4], "text": ["label 1", "label 2"]})
        # add centroids
        store.add_segments(plot, {"x0": [0, 9], "x1": [9, 0], "y0": [0, 0], "y1": [10, 10]})
        store.add_centroids_x(plot, {"x": [1, 2, 3], "y0": [0, 0, 0], "y1": [3, 5, 7]})
        store.add_centroids_y(plot, {"x0": [0, 0, 0], "x1": [1, 2, 3], "y": [3, 5, 7]})
    ...
    def test_add_annotations_image(make_store):
        ...
        image = np.random.randint(0, 100, (10, 10))
        tab_name = "heatmap"
        _, _, plot = store.plot_image(tab_name, {"image": [image]})
        assert "item #0" in store.tabs[tab_name]
        # band annotation
        store.add_span(plot, {"location": 1, "dimension": "width"})
        store.add_span(plot, {"location": 1, "dimension": "height"})
    ...
    def test_get_layout(make_store):
        ...
        store.plot_image(tab_name, {"image": [image]}, layout_name=grid_name)
...
class TestCustomPlotStore:
    ...
    def test_add_multiline_spectrum(make_custom_store):
        ...
        tab_name = "spectrum"
        store.add_tab(tab_name)
        store.plot_multiline_spectrum("spectrum", {"xs": [x, x], "ys": [y, y[::-1]]})
        assert "item #0" in store.tabs[tab_name]
    ...
    def test_add_mass_spectrum(make_custom_store):
        ...
        store.add_tab(tab_name)
        _, _, plot = store.plot_mass_spectrum(tab_name, {"x": x, "y": y})
        assert "item #0" in store.tabs[tab_name]
    ...
    def test_add_centroid_mass_spectrum(make_custom_store):
        ...
        tab_name = "mass spectrum"
        store.add_tab(tab_name)
        store.plot_centroid_mass_spectrum(tab_name, {"x": x, "y0": np.zeros_like(y), "y1": y})
        assert "item #0" in store.tabs[tab_name]
    ...
    def test_add_butterfly_mass_spectrum(make_custom_store):
        ...
        store.add_tab(tab_name)
        store.plot_butterfly_mass_spectrum(tab_name, {"x_top": x, "y_top": y, "x_bottom": x, "y_bottom": -y})
        assert "item #0" in store.tabs[tab_name]
        store.plot_butterfly_mass_spectrum(
            tab_name, {"x_top": x, "y_top": y, "x_bottom": x, "y_bottom": -y}, add_legend=True
        )
        assert "item #0" in store.tabs[tab_name]
    ...
    def test_add_mobilogram(make_custom_store):
        ...
        tab_name = "mass spectrum"
        store.add_tab(tab_name)
        store.plot_mobilogram(tab_name, {"x": x, "y": y})
        assert "item #0" in store.tabs[tab_name]
    ...
    def test_add_butterfly_mobilogram(make_custom_store):
        ...
        store.add_tab(tab_name)
        store.plot_butterfly_mobilogram(tab_name, {"x_top": x, "y_top": y, "x_bottom": x, "y_bottom": -y})
        assert "item #0" in store.tabs[tab_name]
